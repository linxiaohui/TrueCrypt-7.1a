...
namespace TrueCrypt
{
		...
		StartBackgroundTask (false)
	{
		...
		if (parser.Found (L"random-source", &str))
			ArgRandomSourcePath = FilesystemPath ((string)str);
		...
		if (parser.GetParamCount() > 0)
		{
			...
			if (param1IsFile)
			{
				ArgFilePath.reset (new FilePath ((string)parser.GetParam (0)));
			}
		}
	...
	VolumeInfoList CommandLineInterface::GetMountedVolumes (const wxString &mountedVolumeSpec) const
	{
		...
		foreach (shared_ptr <VolumeInfo> volume, volumes)
		{
			...
			else if (wxString ((string)volume->Path) == pathFilter.GetFullPath())
			{
				filteredVolumes.push_back (volume);
			}
			else if (wxString ((string)volume->MountPoint) == pathFilter.GetFullPath()
				|| (wxString ((string)volume->MountPoint) + wxFileName::GetPathSeparator()) == pathFilter.GetFullPath())
			{
				filteredVolumes.push_back (volume);
			}
		}
